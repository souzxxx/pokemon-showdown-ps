...
const jwtVerifier = ({ issuerBaseURL = process.env.ISSUER_BASE_URL, jwksUri = process.env.JWKS_URI, issuer = process.env.ISSUER, audience = process.env.AUDIENCE, secret = process.env.SECRET, tokenSigningAlg = process.env.TOKEN_SIGNING_ALG, mcd, agent, cooldownDuration = 30000, timeoutDuration = 5000, cacheMaxAge = 600000, cache, clockTolerance = 5, maxTokenAge, strict = false, validators: customValidators, }) => {
    ...
    const verifyAndValidateJwt = async (jwt, issuerValue, jwksUriValue, secretValue, algValue, allowedSigningAlgsValue) => {
        const validators = {
    ...
    return async (jwt, requestContext) => {
        ...
        try {
            if (mcd) {
                ...
                return await verifyAndValidateJwt(jwt, tokenIssuer, finalJwksUri, finalSecret, finalAlg, allowedSigningAlgs);
            }
            if (issuerBaseURL) {
            ...
            return await verifyAndValidateJwt(jwt, issuer, jwksUri, secret, tokenSigningAlg, allowedSigningAlgs);
        }
        catch (e) {
